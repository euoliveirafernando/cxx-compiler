...
static type * parse_function(type * i)
{
    ...
    j->data = create_list();
    if (declaration(kind))
        parse_ansi_args(j->data);
    else do
        parse_old_kr_args(j->data);
    while (next_token(','));
    expect(')');
...
static bool is_old_kr(list * x)
{
    if (x != NULL)
        if (x->next != NULL)
            if (is_old_kr_null_type(x->next->data))
                return true;
    return false;
}

static bool defined(variable * v)
{
    if (v->type->type != function)
        return false;
    else if (declaration(kind)) {
        if (!is_old_kr(v->type->data))
            error("old kr prototype");
        do old_kr_args(v->type->data);
        while (declaration(kind));
        expect('{');
        return true;
    } else if (next_token('{'))
...
{
    if (x != NULL)
        for (node * i = x->next; i != NULL;
            i = i->next)
            old_kr_default_to_int(i->data);
}

static void realign(table * t, variable * v)
{
    default_to_int(v->type->data);
    table_insert_list(t, v->type->data);
}
